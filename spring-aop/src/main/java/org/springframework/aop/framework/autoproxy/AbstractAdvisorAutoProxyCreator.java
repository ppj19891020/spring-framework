...
/**
 * AbstractAdvisorAutoProxyCreator是一个通用的自动代理创建器，其设计目的就是检测每个bean上适用的Advisor并据此为该bean创建AOP代理。
 * 该类的子类可以重写方法findCandidateAdvisors()用于查找容器中所有的Advisor，这些Advisor有可能用于每个bean(是否真正适用，取决于匹配条件，
 * 比如通过所定义的pointcut表达式求值匹配等手段)。子类也可以重写方法shouldSkip()决定是否要排除对某些bean创建代理。
